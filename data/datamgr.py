...
class RandomLabeledTargetDataManager(DataManager):
  ...
  def get_data_loader(self, data_file, data_file_miniImagenet, aug): #parameters that would change on train/val set
    ...
    data_loader_params = dict(batch_size = self.batch_size, shuffle = True, num_workers = 4, pin_memory = True)  #8
    data_loader = torch.utils.data.DataLoader(dataset, **data_loader_params)
...
class SetDataManager(DataManager):
  ...
  def get_data_loader(self, data_file, aug): #parameters that would change on train/val set
    ...
    data_loader_params = dict(batch_sampler = sampler,  num_workers=4)  #4
    data_loader = torch.utils.data.DataLoader(dataset, **data_loader_params)
    return data_loader
